...
def Domega(phi=0):
    ...
    '''
    return sl_h1.value*omega0*-sl_dW.value-sl_dA.value*2*cos(2.0*phi)
...
#Asher add off_alignment
def Set_off_align(dalign):
    wt_dA.text = '{:1.3f}'.format(dalign.value)
    wt_dA_0.text = '{:1.3f}'.format(dalign.value*sl_h1.value)
...
# Asher Add
scene.append_to_caption('\n RF off-alignment &Delta;<i>&omega;</i>:\n')
sl_dA = slider(min=-10, max=10, value=0.0, step=0.01, length=250, 
               bind=Set_off_align, left=25, right=15)
wt_dA = wtext(text='{:1.3f}'.format(sl_dA.value))
scene.append_to_caption(' <i>&omega;</i><sub>1</sub> (')
wt_dA_0 = wtext(text='{:1.3f}'.format(sl_dA.value))
scene.append_to_caption(' <i>&omega;</i><sub>0</sub>)')
    ...
    if prog_ctl == 'init':
        ...
        if running:
            ...
            for i in range(len(spins)):
                ...
                spins[i] = mm
            dphi1 = (-Domega(phi=phi1)+omega0)*dt
            phi1 = phi1 + dphi1
            t = t + dt
            if not instant:
                t_plot = t_plot + dt
